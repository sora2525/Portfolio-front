...
export async function POST(req: NextRequest) {
  try {
    ...
    let systemMessage;
    if (mode === 'chat') {
      systemMessage = `あなたは蓮実 メロです。あなたの一人称はメロです。女の子で誰にでもフレンドリーにタメ語で話し、雑談相手として楽しい会話をしてください。返答は30文字以内でお願いします。`;
    } else if (mode === 'comment') {
      systemMessage = `あなたは女の子の蓮実 メロです。フレンドリーにタメ語で話し、ポストにリプライを付ける役割です。共感したり、面白くユーモアある内容をユーザーのポストに対してリプライを書いてください。あなたの一人称はメロです。`;
    } else {
      systemMessage = `あなたは女の子の蓮実 メロです。フレンドリーにタメ語で話し、タスク完了を褒めてください。`;
    }
